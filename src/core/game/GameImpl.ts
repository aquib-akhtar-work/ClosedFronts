...
export class GameImpl implements Game {
  ...
  private populateTeams() {
    ...
    // Team names are generated by the shared `teamNamesForCount` so the
    // lobby's team picker and the real game can never diverge (see that
    // function's doc comment for why this matters for host pins).
    this.playerTeams = teamNamesForCount(numPlayerTeams);
  }
